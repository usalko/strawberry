...
def _convert_from_tortoise_orm_model_to_strawberry_type(
    ...
    if isinstance(type_, StrawberryOptional):
        if data is None:
            return data
        return _convert_from_tortoise_orm_model_to_strawberry_type(
            type_.of_type, data_from_model=data, extra=extra
        )
    if isinstance(type_, StrawberryUnion):
        for option_type in type_.types:
            if hasattr(option_type, "_tortoise_orm_model_type"):
                source_type = option_type._tortoise_orm_model_type
            else:
    ...
    if isinstance(type_, StrawberryList):
        ...
        for index, item in enumerate(data):
            items.append(
                _convert_from_tortoise_orm_model_to_strawberry_type(
                    type_.of_type,
    ...
    if hasattr(type_, "_type_definition"):
        ...
        if hasattr(type_, "from_tortoise_orm_model"):
            return type_.from_tortoise_orm_model(data_from_model, extra)
        return convert_tortoise_orm_model_to_strawberry_class(
...
def convert_tortoise_orm_model_to_strawberry_class(cls, *, model_instance=None, extra=None):
    extra = extra or {}
    kwargs = {}
    ...
    for field in cls._type_definition.fields:
        ...
        # method of the class
        if field.init:
            kwargs[python_name] = _convert_from_tortoise_orm_model_to_strawberry_type(
...
def convert_strawberry_class_to_tortoise_orm_model(obj):
    if hasattr(obj, "to_tortoise_orm_model"):
        return obj.to_tortoise_orm_model()
    elif dataclasses.is_dataclass(obj):
        result = []
        for f in dataclasses.fields(obj):
            value = convert_strawberry_class_to_tortoise_orm_model(getattr(obj, f.name))
            result.append((f.name, value))
        return dict(result)
    elif isinstance(obj, (list, tuple)):
        # Assume we can create an object of this type by passing in a
        # generator (which is not true for namedtuples, not supported).
        return type(obj)(convert_strawberry_class_to_tortoise_orm_model(v) for v in obj)
    elif isinstance(obj, dict):
        return type(obj)(
            (
                convert_strawberry_class_to_tortoise_orm_model(k),
                convert_strawberry_class_to_tortoise_orm_model(v),
            )
