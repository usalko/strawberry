...
from .conversion import (
    convert_tortoise_orm_model_to_strawberry_class,
    convert_strawberry_class_to_tortoise_orm_model,
)
from .exceptions import MissingFieldsListError
from .fields import replace_types_recursively
from .utils import (
    DataclassCreationFields,
    ensure_all_auto_fields_in_tortoise_orm_model,
...
def get_type_for_field(field: Field, is_input: bool):
...
def _build_dataclass_creation_fields(
    ...
    use_tortoise_orm_model_alias: bool,
) -> DataclassCreationFields:
    field_type = (
    ...
    if (
        ...
        elif field.has_alias and use_tortoise_orm_model_alias:
...
if TYPE_CHECKING:
    from .conversion_types import (
        TortoiseOrmModel,
        StrawberryTypeFromTortoiseOrmModel,
    )


def type(
    model: Type[TortoiseOrmModel],
    *,
    ...
    use_tortoise_orm_model_alias: bool = True,
) -> Callable[..., Type[StrawberryTypeFromTortoiseOrmModel[TortoiseOrmModel]]]:
    def wrap(cls: Any) -> Type[StrawberryTypeFromTortoiseOrmModel[TortoiseOrmModel]]:
        ...
        all_model_fields: List[DataclassCreationFields] = [
            _build_dataclass_creation_fields(
                field, is_input, extra_fields_dict, auto_fields_set, use_tortoise_orm_model_alias
            )
        ...
        ) -> StrawberryTypeFromTortoiseOrmModel[TortoiseOrmModel]:
            return convert_tortoise_orm_model_to_strawberry_class(
                cls=cls, model_instance=instance, extra=extra
            )

        def to_pydantic_default(self, **kwargs) -> TortoiseOrmModel:
            instance_kwargs = {
                f.name: convert_strawberry_class_to_tortoise_orm_model(
                    getattr(self, f.name)
                )
...
def input(
    ...
    *,
    ...
    return type(
        ...
        use_tortoise_orm_model_alias=use_tortoise_orm_model_alias,
    )
