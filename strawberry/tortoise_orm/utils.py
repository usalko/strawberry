...
def get_default_factory_for_field(
    field: Field,
) -> Union[Any, dataclasses._MISSING_TYPE]:
    """
    Gets the default factory for a tortoise_orm_model field.

    Handles mutable defaults when making the dataclass by
    using tortoise_orm_model's smart_deepcopy
    ...
    if has_factory and has_default:
        default_factory = cast(Any, default_factory)
...
def ensure_all_auto_fields_in_tortoise_orm_model(
    model: Type[Model], auto_fields: Set[str], cls_name: str
) -> Union[NoReturn, None]:
